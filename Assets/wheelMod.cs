...
public class wheelMod : MonoBehaviour {
    ...
    void Update ()
    {
        ...
        {
            //steer tire visual//
            turnObj.transform.localRotation = Quaternion.Euler(wheelObj.transform.localRotation.x,vehicleObj.GetComponent<vehicleControllerMod>().inputX*33f,wheelObj.transform.localRotation.z);

            //spin tire visual//
            wheelObj.transform.Rotate((vehicleObj.GetComponent<vehicleControllerMod>().zVel*Time.deltaTime*100f),0f,0f);
    ...
    void setTouchGround(bool _bool)
    {
        ...
        if(wheelLB)
        {
            vehicleObj.GetComponent<vehicleControllerMod>().touchLB = touchGround;
        }
        else if(wheelLF)
        {
            vehicleObj.GetComponent<vehicleControllerMod>().touchLF = touchGround;
        }
        else if(wheelRB)
        {
            vehicleObj.GetComponent<vehicleControllerMod>().touchRB = touchGround;
        }
        else if(wheelRF)
        {
            vehicleObj.GetComponent<vehicleControllerMod>().touchRF = touchGround;
        }
